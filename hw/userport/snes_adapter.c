...
void snes_adapter_latch(pio_t* pio, uint8_t pin, uint8_t bit) {
    ...
    pio_set_a_pin(pio, SNES_IO_DATA_1, bits & 0x01);
}
...
void snes_adapter_clock(pio_t* pio, uint8_t pin, uint8_t bit) {
    ...
    snes_adapter->bits >>= 1;
    pio_set_a_pin(pio, SNES_IO_DATA_1, snes_adapter->bits & 0x01);
