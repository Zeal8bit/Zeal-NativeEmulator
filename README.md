...
`--console` reads one command per line from `stdin`, which makes it easy to
drive the emulator deterministically from scripts and CI. Supported commands:
...
`run` is based on emulated Z80 T-states, so tests are deterministic and can
run faster than real time. Keys include `A-Z`, `0-9`, `UP`, `DOWN`, `LEFT`,
`RIGHT`, `ENTER`, `ESC`, `SPACE`, `BACKSPACE`, `TAB`, `SHIFT`, `CTRL`, `ALT`
and more.
...
Adding `--debug` turns the console into a headless debugger front-end, using
the same `dbg_t` back-end as the GUI debugger:
